...
from gameEngine import (
    ...
    compute_final_scores, Piece, board_to_hash
)
...
class GameCoordinator:
    ...
    def create_game(self, board_size: str = "small") -> str:
        ...
        # Determine board dimensions and time limits
        if board_size == "small":
            rows, cols = 13, 12
            ...
            rows, cols = 17, 16
            time_limit = 360.0  # 6 minutes
        else:
            rows, cols = 13, 12
            time_limit = 120.0  # 2 minutes (default)
        ...
        game_state = {
            ...
            "players": {
                "circle": {"connected": False, "port": 8181, "time_left": time_limit},
                "square": {"connected": False, "port": 8182, "time_left": time_limit}
            },
            ...
            "game_log": [],
            "board_state_history": [],  # Track board states for repetition detection
            "moves_since_last_check": 0  # Count moves between repetition checks
        }
    ...
    def make_move(self, player: str, move: Dict[str, Any], thinking_time: float = None) -> Dict[str, Any]:
        ...
        if success:
            ...
            # Increment move counter for repetition detection
            game["moves_since_last_check"] += 1
            
            # Check for repetition every 4 moves (2 moves per player)
            if game["moves_since_last_check"] >= 4:
                current_state = board_to_hash(game["board"])
                game["board_state_history"].append(current_state)
                game["moves_since_last_check"] = 0
                
                # Detect 3 consecutive identical board states (stalemate/infinite loop)
